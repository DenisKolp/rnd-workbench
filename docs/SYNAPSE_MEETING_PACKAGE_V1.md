...
Приложение требует ровно один текстовый файл, в имени которого есть
...
`резюме`. Поддерживаемые расширения этих двух файлов: `.txt`, `.md`,
`.markdown`, `.csv`, `.tsv`, `.json`, `.log`, `.xml`; содержимое должно быть
UTF-8. Все остальные видимые файлы считаются вложениями. Скрытые файлы и
служебный каталог `__MACOSX` игнорируются.
...
вложений; общий объём читаемых частей ограничен 250 МБ. Каталог и ZIP ограничены
