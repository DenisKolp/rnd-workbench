...
class AssistantStore:
    ...
    def _migrate(self) -> None:
        ...
        CREATE TABLE IF NOT EXISTS source_relations (
            ...
            metadata TEXT NOT NULL DEFAULT '{}',
            created_at TEXT NOT NULL,
            UNIQUE(source_id, relation_type, related_source_id),
            CHECK(source_id != related_source_id)
        );
        CREATE INDEX IF NOT EXISTS source_relations_source_idx
            ON source_relations(source_id, relation_type, created_at);
        CREATE INDEX IF NOT EXISTS source_relations_related_idx
            ON source_relations(related_source_id, relation_type, created_at);
        CREATE TABLE IF NOT EXISTS source_import_receipts (
            ...
            primary_source_id TEXT REFERENCES sources(id) ON DELETE CASCADE,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            PRIMARY KEY(source_system, external_id),
            CHECK(
                (state='importing' AND claim_token IS NOT NULL
                    AND lease_expires_at IS NOT NULL AND primary_source_id IS NULL)
                OR
                (state='complete' AND claim_token IS NULL
                    AND lease_expires_at IS NULL AND primary_source_id IS NOT NULL)
            )
        );
    ...
    def _seed(self) -> None:
        ...
        with self.transaction() as connection:
            ...
            capabilities = [
                ("dictation", "Диктовка", "Голос", "Удерживайте системную push-to-talk клавишу: локальный Whisper вставит текст в активное поле", "connected", "safe"),
                ...
                ("synapse-package", "Импорт встреч eXpress (Синапс)", "Встречи", "Fallback-импорт транскрипта, описания, метаданных и вложений с проверяемым происхождением", "connected", "safe"),
                ("email", "Почта", "Внешние системы", "Корпоративная почта: требуется подключение", "not_connected", "confirm"),
                ("synapse", "eXpress Recordings Bot / BotX", "Внешние системы", "Автоматический сбор встреч eXpress: требуется корпоративное подключение", "not_connected", "confirm"),
                ("calendar", "Календарь", "Внешние системы", "Корпоративный календарь: требуется подключение", "not_connected", "confirm"),
                ("corporate", "Корпоративные системы", "Внешние системы", "Другие рабочие API: требуется отдельное подключение", "not_connected", "confirm"),
            ]
            connection.executemany(
    ...
    def source_import_receipt(
        self,
        source_system: str,
        external_id: str,
    ) -> dict[str, Any] | None:
        """Return the durable cross-process receipt for one imported package."""

        source_system = source_system.strip().casefold()
        external_id = external_id.strip()
        if not source_system or not external_id:
            return None
        rows = self._rows(
            """
            SELECT * FROM source_import_receipts
    ...
    def claim_source_import(
        ...
        *,
        ...
        """Claim one package identity across independent SQLite connections.

        SQLite's unique primary key is the actual exclusion mechanism. A live
        unexpired claimant is never replaced. An expired lease may be reclaimed
        by an explicit retry, which is reported to the caller so it can remove
        only the previous incomplete graph before writing new rows.
        """

        source_system = source_system.strip().casefold()
        external_id = external_id.strip()
        package_fingerprint = package_fingerprint.strip().casefold()
        if not source_system or len(source_system) > 64:
            raise ValueError("Некорректная система происхождения импорта")
        if not external_id or len(external_id) > 256:
            raise ValueError("Некорректный внешний идентификатор импорта")
        if not re.fullmatch(r"[a-f0-9]{64}", package_fingerprint):
            raise ValueError("Некорректный fingerprint импорта")
        if not isinstance(lease_seconds, int) or not 1 <= lease_seconds <= 86_400:
            raise ValueError("Некорректный срок lease импорта")
        ...
        with self.transaction() as connection:
            inserted = connection.execute(
                """
                INSERT INTO source_import_receipts(
                    source_system, external_id, package_fingerprint,
                    workspace_id, state, claim_token, lease_expires_at,
            ...
            if inserted.rowcount == 1:
                ...
                if current is None:
                    raise RuntimeError("Receipt импорта исчез во время claim")
                if current["package_fingerprint"] != package_fingerprint:
                    raise ValueError(
                        "Пакет с таким идентификатором уже импортирован с другим содержимым"
                    )
                if current["workspace_id"] != workspace_id:
                    raise ValueError(
                        "Пакет с таким идентификатором закреплён за другим Workspace"
                    )
                if current["state"] == "complete":
                    return {**dict(current), "status": "complete", "recovered": False}
                if str(current["lease_expires_at"]) > now_text:
                    return {
                        **dict(current),
                        "claim_token": None,
                        "status": "busy",
                ...
                updated = connection.execute(
                    ...
                    WHERE source_system=? AND external_id=?
                      AND state='importing' AND lease_expires_at<=?
                    """,
                    (
                        claim_token,
                        lease_expires_at,
                ...
                if updated.rowcount != 1:
                    ...
                    if current is None:
                        raise RuntimeError("Receipt импорта исчез во время reclaim")
                    status = "complete" if current["state"] == "complete" else "busy"
                    return {
                        **dict(current),
                        "claim_token": None,
                        "status": status,
            ...
            if current is None:
                raise RuntimeError("Не удалось прочитать созданный receipt импорта")
            return {**dict(current), "status": status, "recovered": recovered}

    def renew_source_import_claim(
        self,
        source_system: str,
        external_id: str,
        claim_token: str,
        *,
        ...
        """Extend a live import lease without changing its fencing token."""
        ...
        with self.transaction() as connection:
            updated = connection.execute(
                ...
                WHERE source_system=? AND external_id=?
                  AND state='importing' AND claim_token=?
                """,
                (
                    lease_expires_at,
                    now_text,
            ...
            if updated.rowcount != 1:
                raise RuntimeError("Lease импорта больше не принадлежит этому процессу")
        receipt = self.source_import_receipt(source_system, external_id)
        if receipt is None:
            raise RuntimeError("Receipt импорта исчез после renew")
        return receipt
    ...
    def complete_source_import(
        ...
        """Atomically turn the caller's live claim into a durable receipt."""
        ...
        with self.transaction() as connection:
            ...
            if source is None:
                raise KeyError(primary_source_id)
            try:
                metadata = json.loads(source["metadata"] or "{}")
            except (TypeError, json.JSONDecodeError) as error:
                raise ValueError("Primary source содержит некорректный provenance") from error
            provenance = metadata.get("provenance")
            if not isinstance(provenance, dict) or (
                source["workspace_id"] != workspace_id
                or str(provenance.get("source_system") or "").casefold()
                != source_system
                or str(provenance.get("external_id") or "") != external_id
                or str(provenance.get("package_fingerprint") or "")
                != package_fingerprint
                or provenance.get("part_role") != "transcript"
            ):
                raise ValueError("Primary source не соответствует claim импорта")
            updated = connection.execute(
                """
                UPDATE source_import_receipts
                SET state='complete', claim_token=NULL, lease_expires_at=NULL,
                    primary_source_id=?, updated_at=?
                WHERE source_system=? AND external_id=?
                  AND package_fingerprint=? AND workspace_id=?
                  AND state='importing' AND claim_token=?
                """,
                (
                    primary_source_id,
            ...
            if updated.rowcount != 1:
                ...
                if (
                    ...
                    or current["primary_source_id"] != primary_source_id
                    or current["package_fingerprint"] != package_fingerprint
                ):
                    raise RuntimeError(
                        "Claim импорта потерян до фиксации complete receipt"
                    )
        receipt = self.source_import_receipt(source_system, external_id)
        if receipt is None:
            raise RuntimeError("Receipt импорта исчез после complete")
        return receipt

    def release_source_import_claim(
        self,
        source_system: str,
        external_id: str,
        claim_token: str,
    ) -> bool:
        """Release only the caller's unfinished claim; completed receipts persist."""

        with self.transaction() as connection:
            deleted = connection.execute(
                """
                DELETE FROM source_import_receipts
                WHERE source_system=? AND external_id=?
                  AND state='importing' AND claim_token=?
                """,
    ...
    def find_source_by_provenance(
        ...
        """Find one primary source by safe imported-origin metadata.

        JSON1 is intentionally not required here: old user databases and test
        SQLite builds can still open the store. Package imports are rare, so a
        complete scan preserves idempotency even after a long-running profile
        has accumulated thousands of meetings.
        """

        for row in self.sources_by_provenance(source_system, external_id):
            provenance = row["metadata"]["provenance"]
            if (
                row["kind"] == "meeting"
                and str(provenance.get("part_role") or "") == "transcript"
            ):
                return row
        return None

    def sources_by_provenance(
        self,
        source_system: str,
        external_id: str,
    ) -> list[dict[str, Any]]:
        """Return every package source, including relation-orphaned crash residue."""

        expected_system = source_system.strip().casefold()
        expected_id = external_id.strip()
        if not expected_system or not expected_id:
            return []
        result: list[dict[str, Any]] = []
        for row in self._rows("SELECT * FROM sources ORDER BY created_at, id"):
            try:
            ...
            if not isinstance(provenance, dict):
                continue
            if (
                str(provenance.get("source_system") or "").casefold()
                == expected_system
                and str(provenance.get("external_id") or "") == expected_id
            ):
    ...
    def add_source_relation(
        ...
        *,
        ...
        if not relation_type or len(relation_type) > 80:
            raise ValueError("Некорректный тип связи источников")
        if source_id == related_source_id:
            raise ValueError("Источник нельзя связать с самим собой")
        source = self.get_source(source_id)
        related = self.get_source(related_source_id)
        if source["workspace_id"] != related["workspace_id"]:
            raise ValueError("Связанные источники должны принадлежать одному Workspace")
        relation_id = new_id()
        now = utc_now()
        ...
        with self.transaction() as connection:
            connection.execute(
                ...
                INSERT INTO source_relations(
                    id, source_id, relation_type, related_source_id, metadata, created_at
                ) VALUES (?, ?, ?, ?, ?, ?)
                ON CONFLICT(source_id, relation_type, related_source_id) DO UPDATE SET
        ...
        rows = self._rows(
            """
            SELECT sr.*, s.title AS related_title, s.kind AS related_kind,
                   s.path AS related_path
            FROM source_relations sr
            JOIN sources s ON s.id=sr.related_source_id
            WHERE sr.source_id=? AND sr.relation_type=? AND sr.related_source_id=?
            """,
            (source_id, relation_type, related_source_id),
        )
        relation = rows[0]
    ...
    def source_relations(
        ...
        *,
        ...
        clause = ""
        if relation_type:
            clause = " AND sr.relation_type=?"
            parameters.append(relation_type)
        rows = self._rows(
            f"""
            SELECT sr.*, s.title AS related_title, s.kind AS related_kind,
                   s.path AS related_path, s.classification AS related_classification
            FROM source_relations sr
            JOIN sources s ON s.id=sr.related_source_id
            WHERE sr.source_id=?{clause}
            ORDER BY sr.relation_type, sr.created_at, sr.related_source_id
            """,
            tuple(parameters),
        )
        for row in rows:
    ...
    def delete_source(self, source_id: str) -> dict[str, Any]:
        """Delete a source and its importer-owned package graph when applicable."""

        return self.delete_source_package_aware(source_id)

    def delete_source_package_aware(self, source_id: str) -> dict[str, Any]:
        """Delete a Synapse primary with owned related sources, related-first.

        Only sources linked by the reserved Synapse relation types or carrying
        matching importer provenance with ``primary_source_id`` are included.
        File movement remains constrained by ``_trash_managed_paths`` to the
        application's managed roots, so an original/exported package is never
        moved even if a malformed source row points to it.
        """

        source = self.get_source(source_id)
        sources, receipt_identity = self._source_delete_group(source)
        managed_paths = [
            path
            for package_source in sources
            for path in self._source_managed_paths(package_source)
        ]
        meeting_ids = {
            str(row["id"])
            for package_source in sources
        ...
        moved = self._trash_managed_paths(managed_paths)
        try:
            with self.transaction() as connection:
                ...
                for package_source in sources:
                    self._delete_source_rows(connection, str(package_source["id"]))
        except BaseException:
            self._restore_trashed_paths(moved)
            raise
        self.audit(
            ...
            json.dumps(
                {
                    ...
                    "package_aware": receipt_identity is not None,
                },
                ensure_ascii=False,
                sort_keys=True,
            ),
        ...
        return {
            ...
            "trashed_files": len(moved),
            "deleted_sources": len(sources),
            "deleted_source_ids": [str(item["id"]) for item in sources],
            "deleted_meeting_ids": sorted(meeting_ids),
            "package_aware": receipt_identity is not None,
        }

    def _source_delete_group(
        self,
        source: dict[str, Any],
    ) -> tuple[list[dict[str, Any]], tuple[str, str] | None]:
        """Resolve only importer-owned descendants; keep the primary last."""

        metadata = source.get("metadata")
        provenance = metadata.get("provenance") if isinstance(metadata, dict) else None
        if not isinstance(provenance, dict) or (
            source.get("kind") != "meeting"
            or str(provenance.get("source_system") or "").casefold() != "synapse"
            or provenance.get("part_role") != "transcript"
        ):
            return [source], None
        ...
        fingerprint = str(provenance.get("package_fingerprint") or "").strip()
        if not external_id or not re.fullmatch(r"[a-f0-9]{64}", fingerprint):
            return [source], None

        owned: dict[str, dict[str, Any]] = {}
        relation_rows = self._rows(
            """
            SELECT related_source_id FROM source_relations
            WHERE source_id=?
              AND relation_type IN ('synapse.description', 'synapse.attachment')
            ORDER BY created_at, related_source_id
            """,
            (source["id"],),
        )
        relation_ids = {str(row["related_source_id"]) for row in relation_rows}
        for candidate in self.sources_by_provenance(source_system, external_id):
            if candidate["id"] == source["id"]:
                continue
            candidate_provenance = candidate["metadata"].get("provenance")
            if not isinstance(candidate_provenance, dict):
                continue
            role = str(candidate_provenance.get("part_role") or "")
            expected_kind = {
            ...
            if (
                ...
                or candidate.get("workspace_id") != source.get("workspace_id")
                or candidate_provenance.get("package_fingerprint") != fingerprint
                or candidate_provenance.get("primary_source_id") != source["id"]
            ):
                continue
            if candidate["id"] in relation_ids or candidate_provenance.get(
                "primary_source_id"
            ) == source["id"]:
                owned[str(candidate["id"])] = candidate

        related = sorted(
            owned.values(),
            key=lambda item: (str(item.get("created_at") or ""), str(item["id"])),
        )
        return [*related, source], (source_system, external_id)
