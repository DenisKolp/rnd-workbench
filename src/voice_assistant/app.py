...
from .backends import MLXChat, OpenAICompatibleChat, WhisperSTT, create_tts
...
class VoiceAssistant:
        ...
        try:
            stream = (
                active_chat.stream_reply(
                    ...
                    system_prompt=self._external_system_prompt(),
            ...
            for token in stream:
                ...
                if speak and not speech_cancel.is_set():
    ...
    def _external_system_prompt(self) -> str:
        """Make the external trust boundary explicit at system priority."""

        return (
            self.config.llm.system_prompt.rstrip()
            ...
            "локальными на Mac. Не утверждай, что весь ответ обработан локально."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="local-voice",
        description="Полностью локальный голосовой помощник для Apple Silicon.",
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def _loopback_test(assistant: VoiceAssistant) -> int:
    ...
    phrase = "Привет, Денис. Локальный голосовой помощник слышит и говорит по-русски."
