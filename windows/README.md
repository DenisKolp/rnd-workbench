...
действия настроек остаются видимыми, длинные подписи кнопок не переполняются,
панель инструментов сохраняет вертикальную компоновку, а техническая
диагностика пилота свёрнута по умолчанию.
...
диктовки и meeting-сценариев, время первого результата и оценка 1–5. JSON-отчёт
не содержит запросы, транскрипты, документы, сырые даты или идентификаторы.
