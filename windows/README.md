...
milestone **0.9.15**, но не обозначает готовую Windows-поставку.
...
| Настраиваемые дайджесты | Готово в исходниках | `/digest неделя только риски и поручения` выполняется локально без LLM; фильтр и ссылки сохраняются с материалом |
| Фоновые автоматизации | Ограниченно готово | Временные расписания локальных `/digest` выполняются, пока приложение скрыто в области уведомлений; автозапуск с Windows не включён, событийные и удалённые действия отклоняются |
...
увеличиваться по высоте и переносить подпись вместо обрезания. Основные текстовые
кнопки имеют высоту не менее 38 px, компактные вкладки и свёрнутые разделы — не
менее 36 px; критически мелкие подписи действий увеличены. Переход назван
...
Раздел «Автоматизации» свёрнут по умолчанию и не добавляет кнопок в компактный
голосовой виджет. В нём можно выбрать период и состав дайджеста, расписание,
затем изменить, выключить или удалить правило. Кнопка закрытия полного окна
честно названа «Скрыть в область уведомлений»: backend и локальный планировщик
остаются активны в текущей пользовательской сессии. Это не автозапуск ОС и не
служба, работающая после явного пункта «Выход».
...
Граница фонового выполнения и причины session-only режима зафиксированы в
[`ADR-0002`](../docs/ADR-0002-WINDOWS-SESSION-BACKGROUND.md).
...
Нормализованный пакет принимает текстовую или DOCX-стенограмму. Серверный модуль
BotX v4 уже проверяет входящий JWT и строит безопасный Files API plan, но он не
является развёрнутым endpoint и не меняет статус интеграции на connected.
