...
milestone **0.9.13**, но не обозначает готовую Windows-поставку.
...
| Preflight устройства | Готово в исходниках | 12 content-free проверок показывают реальные блокеры, предупреждения, crash-free после 20 завершений и девять обязательных voice SLO; результат можно обновить в полном окне |
| Цифровая WER-проверка | Готово в исходниках | В полном диагностическом блоке 10 фиксированных фраз локально проходят TTS→STT; сохраняются только WER обычной и корпоративной речи, без аудио и транскриптов |
...
Production renderer имеет отдельные контракты compact 410×420 и full 1120×760:
страница не прокручивается, длинные задачи переносятся, а текстовые кнопки могут
увеличиваться по высоте и переносить подпись вместо обрезания. Переход назван
«Развернуть»/«Виджет». Компактная голосовая подсказка не перегружает экран, а
техническая диагностика пилота и WER-проверка свёрнуты по умолчанию. Свежая
визуальная проверка на реальном Windows-устройстве всё ещё обязательна.

Источники и материалы в полном окне сведены в один свёрнутый блок. Для ответа
доступен переход к использованному фрагменту по сохранённым координатам; рядом
видна классификация данных. Материалы показывают версии и provenance, выбранная
старая версия восстанавливается как новая текущая. Удаление источника или
материала требует подтверждения и использует recoverable trash-семантику.
Компактный voice-first экран не получил постоянной библиотеки или панели
управления: контекстная ссылка появляется только у ответа, который реально
использовал источник.

Поле чата предлагает до шести скиллов после `/` и источников после `@`; выбор
работает мышью, клавишами ↑/↓ и Tab/Enter. Отдельной кнопки или панели в
компактном голосовом режиме нет. План, источники и материалы объединены в один
свёрнутый «Контекст задачи». Фразы «добавь в план…», «замени шаг 2 на…» и
«удали шаг 2» применяются локально до выбора LLM-маршрута, сохраняют аудит и
могут дать один короткий голосовой ответ через уже настроенный TTS.
...
успешного голосового потока. Отдельная локальная квалификация измеряет WER на
пяти обычных и пяти корпоративных фразах; это программный loopback, а не
акустическая проверка динамиков и микрофона. CI уже собирает voice-ready portable
