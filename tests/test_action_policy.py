...
def test_corporate_workflow_actions_are_detected_and_staged_in_spoken_order(
    tmp_path,
) -> None:  # noqa: ANN001
    store = make_store(tmp_path)
    orchestrator = LocalOrchestrator(store)
    turn = orchestrator.prepare_turn(
        "Создай задачу в Jira, создай карточку в Kaiten, затем обнови страницу в Confluence",
        workspace_id=store.default_workspace_id(),
        task_id=None,
    )
    ...
    assert all(item["confirmation_policy"] == "explicit" for item in approvals)
    assert all(item["status"] == "pending" for item in approvals)
    payloads = [json.loads(item["payload"]) for item in approvals]
    assert [item["integration"] for item in payloads] == [
        "jira",
        "kaiten",
        "confluence",
    ]
    assert all(item["production_connector"] is False for item in payloads)
    assert all("предпросмотра" in item["preview"]["summary"] for item in payloads)
...
def test_rejection_is_final_and_cancels_later_workflow_steps(tmp_path) -> None:  # noqa: ANN001
    ...
    rejected = store.resolve_approval(first["id"], "rejected", actor="test-user")
    store.cancel_approval_dependents(first["id"], actor="system")

    assert rejected["status"] == "rejected"
    assert rejected["resolved_by"] == "test-user"
...
def test_error_can_be_edited_and_replanned_with_new_revision(tmp_path) -> None:  # noqa: ANN001
    ...
    store.resolve_approval(approval["id"], "approved", actor="test-user")
