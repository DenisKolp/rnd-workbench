...
def test_project_config_loads() -> None:
    ...
    assert config.tts.chunk_duration_s == 1.5
    ...
    assert config.assistant.max_tts_chars == 220
    assert config.assistant.max_tts_segments == 1
    ...
    assert "push-to-talk" in config.llm.system_prompt
    assert "Пакет встречи из eXpress" in config.llm.system_prompt
    assert "BotX API" in config.llm.system_prompt
