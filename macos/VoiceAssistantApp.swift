...
struct AutomationsView: View {
    ...
    var body: some View {
        HSplitView {
            ScrollView { VStack(alignment: .leading, spacing: 12) { Text(editingID == nil ? "Новая автоматизация" : "Изменить автоматизацию").font(.headline); TextField("Название", text: $name); TextEditor(text: $prompt).frame(height: 110).overlay(RoundedRectangle(cornerRadius: 6).stroke(.quaternary)); Text("Для локального настраиваемого дайджеста: /digest неделя только риски и поручения").font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true); TextField("ежедневно 09:00", text: $schedule); Text("Поддерживается: ежедневно HH:MM, каждую пятницу HH:MM, once YYYY-MM-DD HH:MM или «при новом источнике»").font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true); Button(editingID == nil ? "Создать" : "Сохранить") { if let id = editingID { controller.updateAutomation(id: id, name: name, prompt: prompt, schedule: schedule) } else { controller.createAutomation(name: name, prompt: prompt, schedule: schedule) }; editingID = nil; name = ""; prompt = ""; schedule = "ежедневно 09:00" }.buttonStyle(.borderedProminent).disabled(name.isEmpty || prompt.isEmpty); if editingID != nil { Button("Отмена") { editingID = nil; name = ""; prompt = ""; schedule = "ежедневно 09:00" } } }.padding(20) }.frame(minWidth: 300, idealWidth: 340)
