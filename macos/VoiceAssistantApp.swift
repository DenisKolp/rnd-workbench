...
final class BackendController: ObservableObject {
    ...
    var isCorporateLLMActive: Bool {
        isExternalLLMActive && externalProviderType == "corporate"
    }
    ...
    var canImportMeetingAudio: Bool {
        isReady && !meetingAudioImportInProgress && !isSessionActive
            && !isVoiceStartPending && !isLLMTurnPending && !state.isBusy
    }
    ...
    func toggleSession() {
        if isSessionActive { send(["command": "stop"]) }
        else {
    ...
    func saveMemory(title: String, content: String) { send(["command": "save_memory", "title": title, "content": content]) }
    func updateMemory(id: String, title: String, content: String) {
        send(["command": "update_memory", "id": id, "title": title, "content": content])
    }
    ...
    func configureExternalLLM(
        ...
        do {
            ...
            pendingLLMMode = "external"
            llmConfigurationPending = true
            llmConfigurationError = nil
            externalLLMReady = false
            statusText = "Настраиваю внешнюю модель…"
            send([
    ...
    func deleteExternalLLMAPIKey(baseURL: String) {
        ...
        do {
            ...
            let deletesActiveCredential = isExternalLLMActive && activeEndpoint == canonicalEndpoint
    ...
    private func handleLine(_ line: String) {
        ...
        case "llm_configured":
            ...
            let configuredProviderType = (event["provider_type"] as? String)
                ?? externalProviderType
            let configuredBaseURL = (event["base_url"] as? String) ?? externalLLMBaseURL
            let configuredModel = (event["model"] as? String) ?? externalLLMModel
            ...
            llmConfigurationError = credentialDeletionError
                ?? (configuredReady ? nil : "Настройки сохранены, но модель пока не готова")
        ...
        case "llm_configuration_error":
            ...
            if pendingLLMMode == "external" { externalLLMReady = false }
            llmConfigurationPending = false
            pendingLLMMode = nil
            pendingCredentialDeletionEndpoint = nil
        ...
        case "metric": if event["name"] as? String == "stt" { sttSeconds = number(event["seconds"]) }
        case "search_results":
    ...
    private func applySnapshot(_ data: [String: Any]) {
        ...
        memory = rows(data, "memory").map {
            EntityRecord(
                id: string($0, "id"), title: string($0, "title"),
                subtitle: string($0, "kind"), detail: string($0, "content"),
                classification: string($0, "classification").isEmpty
                    ? "internal" : string($0, "classification")
            )
        }
        ...
        if let llm = data["llm"] as? [String: Any] {
            ...
            let snapshotModel = string(llm, "model")
    ...
    private func hydrateExternalLLMFromSnapshotIfNeeded() {
        ...
        if let validationError = ExternalLLMEndpoint.validationError(baseURL) {
            showLLMConfigurationError("Сохранённый адрес внешней модели недействителен. \(validationError)")
            return
        }
        guard !model.isEmpty else {
            showLLMConfigurationError("Для внешнего провайдера не сохранён идентификатор модели")
            return
        }
        guard let canonicalEndpoint = ExternalLLMEndpoint.canonicalized(baseURL) else {
            showLLMConfigurationError("Не удалось безопасно нормализовать сохранённый адрес провайдера")
            return
        }
        do {
            ...
            guard !apiKey.isEmpty || ExternalLLMEndpoint.isLoopback(canonicalEndpoint) else {
                externalLLMReady = false
                showLLMConfigurationError(
                    "Внешняя модель не активирована: ключ API не найден в Связке ключей macOS. Перейдите в Настройки → Данные и модели."
                )
                return
            }
            llmConfigurationPending = true
            pendingLLMMode = "external"
            llmConfigurationError = nil
            statusText = "Восстанавливаю настройки внешней модели…"
            send([
                "command": "configure_llm",
                "mode": "external",
                "base_url": canonicalEndpoint,
            ...
            externalLLMReady = false
            showLLMConfigurationError("Не удалось прочитать ключ API: \(error.localizedDescription)")
        }
    }
...
struct AssistantWorkspaceView: View {
    ...
    private var sidebar: some View {
        VStack(spacing: 12) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Label(
                        controller.isExternalLLMActive
                            ? "Внешняя модель · контекст передаётся провайдеру"
                            : "Полностью локально",
                        systemImage: controller.isExternalLLMActive ? "network" : "lock.shield.fill"
                    )
                    .font(.caption2)
        ...
        .foregroundStyle(.white)
        .frame(width: 250)
        .background(ZStack { RnDTheme.navy; BrandSupergraphic().opacity(0.72) })
    }
    ...
    var body: some View {
        ...
        .background(
            ZStack(alignment: .topTrailing) {
                ...
                BrandSupergraphic().frame(width: 150, height: 72).opacity(0.08).clipShape(RoundedRectangle(cornerRadius: 16))
            }
        )
        .overlay(RoundedRectangle(cornerRadius: 16).stroke(RnDTheme.line))
...
struct FocusHero: View {
    ...
    var body: some View {
        ZStack {
            ...
            BrandSupergraphic().opacity(0.46)
            HStack(spacing: 22) {
                VStack(alignment: .leading, spacing: 8) {
                    Label(
                        controller.isExternalLLMActive
                            ? "Внешний контур · контекст передаётся провайдеру"
                            : "Локальный рабочий контур",
                        systemImage: controller.isExternalLLMActive ? "network" : "lock.shield.fill"
                    )
                ...
                VStack(spacing: 10) {
                    HStack(spacing: 8) {
                        ...
                        PipelineNode(
                            icon: controller.isExternalLLMActive ? "network" : "brain.head.profile",
                            label: llmLabel,
                            ready: llmReady,
                            accent: controller.isExternalLLMActive ? RnDTheme.red : .white
                        )
                        Image(systemName: "chevron.right").foregroundStyle(.white.opacity(0.5))
    ...
    private var llmLabel: String {
        controller.modelName.isEmpty
            ? (controller.isExternalLLMActive ? "Внешняя LLM" : "Локальная MLX")
    ...
    var body: some View {
        HSplitView {
            ...
            ScrollView { VStack(alignment: .leading, spacing: 18) {
                ...
                HStack { Text("Рабочая память").font(.headline); Spacer(); Button { editingMemoryID = nil; memoryTitle = ""; memoryContent = ""; showMemoryEdit = true } label: { Label("Добавить", systemImage: "plus") } }
                ForEach(controller.memory) { item in EntityRow(item: item, icon: "brain.head.profile", trailing: AnyView(HStack { ClassificationMenu(value: item.classification) { controller.setClassification(entityType: "memory", id: item.id, value: $0) }; Button { editingMemoryID = item.id; memoryTitle = item.title; memoryContent = item.detail; showMemoryEdit = true } label: { Image(systemName: "pencil") }.buttonStyle(.borderless); Button(role: .destructive) { controller.deleteMemory(item.id) } label: { Image(systemName: "trash") }.buttonStyle(.borderless) })) }
        ...
        .sheet(isPresented: $showMemoryEdit) {
            FormSheet(title: editingMemoryID == nil ? "Новая запись памяти" : "Изменить память", primary: "Сохранить", onCancel: { showMemoryEdit = false }, onPrimary: { if let id = editingMemoryID { controller.updateMemory(id: id, title: memoryTitle, content: memoryContent) } else { controller.saveMemory(title: memoryTitle, content: memoryContent) }; showMemoryEdit = false }) { TextField("Название", text: $memoryTitle); TextEditor(text: $memoryContent).frame(height: 150).overlay(RoundedRectangle(cornerRadius: 6).stroke(.quaternary)) }
        }
        .confirmationDialog(
        ...
        HSplitView {
            VStack(spacing: 0) {
                ...
                if visibleMeetings.isEmpty {
                    EmptyState(icon: "person.2.slash", title: "Встречи не найдены", detail: controller.meetings.isEmpty ? "Добавьте аудиозапись или импортируйте готовый транскрипт — распознавание и анализ выполняются локально." : "Измените фильтр по человеку или периоду.")
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 8) {
                Button(action: controller.chooseMeetingAudio) {
                    Label("Добавить аудио", systemImage: "waveform.badge.plus")
                }
                .buttonStyle(.borderedProminent)
                .tint(RnDTheme.navy)
                .fixedSize(horizontal: true, vertical: false)
                .disabled(!controller.canImportMeetingAudio)
                .help("Добавить аудиозапись и локально распознать встречу")
                Button { controller.chooseFile(kind: "meeting") } label: {
                    Label("Транскрипт", systemImage: "doc.badge.plus")
                }
                .buttonStyle(.bordered)
                .fixedSize(horizontal: true, vertical: false)
                .disabled(!controller.canImportMeetingAudio)
                .help("Импортировать готовый транскрипт встречи")
            }
            .controlSize(.small)
...
struct SettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Данные и модели") {
                LabeledContent(
                    ...
                    value: controller.isCorporateLLMActive
                        ? "Корпоративная API"
                        : controller.isExternalLLMActive ? "Внешняя API" : "Локальная MLX"
                )
                LabeledContent("Активная модель", value: controller.modelName)

                Picker("Модель для ответов", selection: $llmModeDraft) {
                    Text("Локальная MLX").tag("local")
                    Text("Корпоративная API").tag("corporate")
                    Text("Внешняя API").tag("external")
                }
                .pickerStyle(.segmented)
                ...
                if controller.llmConfigurationPending {
                    ...
                    Label(message, systemImage: "exclamationmark.triangle.fill")
                        .font(.caption)
                        .foregroundStyle(RnDTheme.red)
                } else if controller.isExternalLLMActive && controller.externalLLMReady {
                    Label(
                ...
                HStack {
                    ...
                    if controller.isExternalLLMActive || llmModeDraft != "local" {
        ...
        .formStyle(.grouped)
        .padding(12)
        .onAppear { loadLLMDrafts(force: true) }
    ...
    private func loadLLMDrafts(force: Bool) {
        ...
        llmModeDraft = controller.llmMode == "local"
            ? "local" : controller.externalProviderType
    ...
    var body: some View {
        VStack(spacing: 0) {
            if let message = controller.speechErrorMessage {
                SpeechFailureBanner(
                    message: message,
                    retryable: controller.speechErrorRetryable,
            ...
            HStack(spacing: 10) {
                ...
                if let value = controller.sttSeconds { Text("STT \(value, specifier: "%.2f")с").font(.caption2.monospacedDigit()).foregroundStyle(.secondary) }
                ...
                Button { send() } label: { Image(systemName: "arrow.up").font(.system(size: 12, weight: .bold)).frame(width: 27, height: 27).background(Circle().fill(RnDTheme.blue)).foregroundStyle(.white) }.buttonStyle(.plain).disabled(controller.composerDraft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || !controller.canSendText)
                Button(action: controller.toggleSession) { Image(systemName: controller.isSessionActive ? "stop.fill" : controller.isVoiceStartPending ? "hourglass" : "mic.fill").font(.system(size: 16, weight: .semibold)).frame(width: 38, height: 38).background(Circle().fill(controller.isSessionActive ? RnDTheme.red : RnDTheme.navy)).foregroundStyle(.white) }.buttonStyle(.plain).disabled(!controller.canToggleVoiceSession).keyboardShortcut(.space, modifiers: [.command]).help(controller.isSessionActive ? "Остановить голосовой режим" : controller.isVoiceStartPending ? "Голосовой режим запускается" : "Начать слушать")
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 10)
        }
...
struct CompactWindowConfigurator: NSViewRepresentable {
    ...
    func updateNSView(_ view: NSView, context: Context) {
        DispatchQueue.main.async {
            ...
            guard context.coordinator.configuredWindow != identifier else { return }
            ...
            window.setFrameOrigin(NSPoint(x: frame.maxX - window.frame.width - 22, y: frame.minY + 22))
        }
    }
}
...
struct CompactAssistantView: View {
    ...
    private var mode: CompactMode { CompactMode(rawValue: modeRaw) ?? .voice }
    private var modeBinding: Binding<CompactMode> {
        Binding(
            get: { mode },
            set: { newMode in
                if newMode == .chat && controller.isSessionActive { controller.toggleSession() }
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .stroke(controller.isExternalLLMActive ? RnDTheme.red.opacity(0.38) : RnDTheme.line)
        )
        .clipShape(RoundedRectangle(cornerRadius: 18, style: .continuous))
        .shadow(color: RnDTheme.navy.opacity(0.18), radius: 22, y: 8)
        .background(CompactWindowConfigurator())
        .foregroundStyle(RnDTheme.ink)
        .tint(RnDTheme.blue)
        .preferredColorScheme(.light)
        .sheet(
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                .foregroundStyle(controller.isExternalLLMActive ? RnDTheme.red : RnDTheme.blue)
                .lineLimit(1)
            }
            ...
            .pickerStyle(.segmented)
            .frame(width: 112)
            Button { openWindow(id: "assistant"); dismissWindow(id: "compact"); NSApp.activate(ignoringOtherApps: true) } label: { Image(systemName: "arrow.up.left.and.arrow.down.right") }.buttonStyle(.borderless).help("Развернуть")
            Button { dismissWindow(id: "compact") } label: { Image(systemName: "xmark") }.buttonStyle(.borderless).help("Закрыть")
        }
        ...
        .background(
            ZStack(alignment: .trailing) {
                RnDTheme.panel
                if controller.isExternalLLMActive { RnDTheme.red.opacity(0.028) }
                BrandSupergraphic().frame(width: 118).opacity(0.13)
            }
        )
    }
    ...
    private var voicePanel: some View {
        HStack(spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text(controller.isSessionActive ? "Голосовой режим включён" : controller.isVoiceStartPending ? "Запускаю микрофон…" : "Начать разговор").font(.headline)
                Text(controller.isSessionActive ? "Говорите естественно — помощника можно перебить." : controller.isVoiceStartPending ? "Подготавливаю локальное распознавание речи." : "Нажмите микрофон. Аудио и распознавание остаются на этом Mac.").font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
    ...
    private var chatPanel: some View {
        VStack(spacing: 7) {
            ScrollViewReader { proxy in
                ...
                .onChange(of: controller.messages.count) { _, _ in if let id = controller.messages.last?.id { proxy.scrollTo(id, anchor: .bottom) } }
            }
            ...
            HStack(spacing: 8) {
                ...
                .accessibilityLabel("Озвучивать ответы")
                .accessibilityValue(speakReplies ? "Включено" : "Выключено")
                Button { send() } label: { Image(systemName: "arrow.up").font(.system(size: 11, weight: .bold)).frame(width: 28, height: 28).background(Circle().fill(RnDTheme.blue)).foregroundStyle(.white) }.buttonStyle(.plain).disabled(controller.composerDraft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || !controller.canSendText)
            }
...
struct MenuBarContent: View {
    ...
    var body: some View {
        ...
        Button("Открыть RnD Workbench") {
            ...
            dismissWindow(id: "compact")
            NSApp.activate(ignoringOtherApps: true)
        }
        Divider()
        Button(controller.isSessionActive ? "Остановить" : "Начать слушать") { controller.toggleSession() }
            .disabled(!controller.canToggleVoiceSession)
        Button("Новая задача") {
            controller.newTask()
            openWindow(id: "assistant")
...
struct LocalVoiceAssistantApp: App {
    ...
    var body: some Scene {
        Window("RnD Workbench", id: "assistant") { AssistantWorkspaceView(controller: controller) }.windowStyle(.hiddenTitleBar).windowResizability(.contentSize).defaultPosition(.center)
        Window("RnD Workbench — компактный режим", id: "compact") { CompactAssistantView(controller: controller) }.windowStyle(.hiddenTitleBar).windowResizability(.contentSize)
